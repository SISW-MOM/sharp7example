     ...
     //   private bool connectedFlag = false;
      ...
      //  public static object ob1;
        ...
        private void ReadArea()
        {
            ...
            Start = System.Convert.ToInt32(txstart.Text);
            Amount = System.Convert.ToInt32(TxtSize.Text);
            Result = Client.ReadArea(Area[CBArea.SelectedIndex], DBNumber, Start, Amount, WordLen[CBWLen.SelectedIndex], Buffer, ref SizeRead);
        
            ShowResult(Result);
            label18.Text = SizeRead.ToString();
            if (Result == 0)
                HexDump(TxtDump, Buffer, SizeRead);
            str1= (DateTime.Now.ToString() + "    "+realnumber(Buffer, SizeRead))+"\n";
            addlinetofile(@"d:\recordss.txt", str1);

        }
        public  void addlinetofile(string path ,string str1)
        {
        ...
        public String realnumber(byte[] bytes, int Size )
        {
            int bytenum = Size;
            int num = (Size )/4;
            float ff;
            Single[] ss=new Single[num];
            string[] str = new string[num];
            string str1="";
            for (int i = 0; i < num; i++)
            {
                
                 byte[] bytesingle = new byte[4];
                bytesingle[0] = bytes[4 * num];
                bytesingle[1] = bytes[4 * num+1];
                bytesingle[2] = bytes[4 * num+2];
                bytesingle[3] = bytes[4 * num+3];
                ff= BitConverter.ToSingle(bytes, 0);
                ff = S7.GetRealAt(bytes, num * 4);
                str1 = str1 + ff.ToString()+"   ";
...
*/
        ...
        private void t_Tick(object sender, EventArgs e)
        {
            string ss = "";
            //  ReadArea();
            ss = DateTime.Now.ToString() + "    " + "ddddddd\n";
            TxtDump.AppendText(ss);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (xmldate1 == null || xmldate1.IsDisposed)
            {
                xmldate1 = new xmldate();             
        ...
        private void FieldBtn_Click_1(object sender, EventArgs e)
        {
